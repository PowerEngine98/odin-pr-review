 ...
 */
export function composeOnFile(id: string, path: string): void {
  ...
  /*
   * The same seam `composing` documents just above, for the same reason. The
   * state module describes a composer's anchor as one rectangle and its line as
   * a number; the composer reads two boxes and treats a missing line as a
   * remark about the whole file — which is exactly what this is. The shape
   * below is what the composer wants; the declaration is what wants changing,
   * and it is not the canvas's to change.
   */
  ui.composer = {
    path,
    side: "RIGHT",
